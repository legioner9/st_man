...
  
 ...
 zcat (1)   zcat  (1)   ( Solaris man: Команды и прикладные программы пользовательского уровня )   zcat  (1)   ( FreeBSD man: Команды и прикладные программы пользовательского уровня ) >>  zcat  (1)   ( Русские man: Команды и прикладные программы пользовательского уровня )   zcat  (1)   ( Linux man: Команды и прикладные программы пользовательского уровня )   zcat  (1)   ( POSIX man: Команды и прикладные программы пользовательского уровня ) 
   
 НАЗВАНИЕ 
...
gzip, gunzip, zcat - сжать или распаковать файлы
 ...
 Gzip  уменьшает размер перечисленных файлов, используя кодирование
Лемпеля-Зива (LZ77). Там, где это возможно, каждый файл заменяется
архивом с расширением  .gz , с таким же владельцем, временем доступа и модификации. (Расширение по умолчанию -   gz  для VMS,  z  для MSDOS, OS/2 FAT, Windows NT FAT и Atari). Если не указано ни одного файла, или как имя файла указан "-", то стандартный поток ввода сжимается в стандартный поток вывода.  Gzip  будет пытаться сжать только обычные файлы. В частности, он проигнорирует символические ссылки.
...
Сжатые файлы могут быть восстановлены в первоначальное состояние командой  gzip -d  или  gunzip  или  zcat . Если первоначальное имя файла, сохранённое в архиве, не допускается файловой системой, в которой он находится, то ему создаётся новое имя из первоначального по правилам этой системы.
 ...
 gunzip  берёт список файлов из командной строки и заменяет каждый
файл с расширением .gz, -gz, .z, -z, _z или .Z и начинающийся правильным заголовком разархивированным файлом без этого расширения.  gunzip  также распознаёт специальные расширения  .tgz  и  .taz  как сокращения от  .tar.gz  и  .tar.Z  соответственно. Сжимая файл,  gzip  по необходимости использует расширение  .tgz  вместо усечения имени у файла с расширением  .tar .
 

 gunzip  на данный момент может распаковывать архивы, созданные  gzip ,  zip ,  compress ,  compress -H  или  pack . Распознавание формата архива автоматическое. Используя первые два формата,  gunzip  проверяет 32-битный CRC. Для формата  pack gunzip  проверяет длину разархивированного файла. Стандартный формат  compress  не был рассчитан на возможность проверки целостности архива. Тем не менее,  gunzip  в некоторых ситуациях может распознать повреждённый .Z-файл. Если вы получаете сообщение об ошибке в процессе распаковки .Z-файла, не думайте, что он не повреждён, если стандартный  uncompress  на него не жалуется. Это обычно происходит из-за того, что  uncompress  не проверяет целостность архива и достаёт из него только кучу мусора. На системе SCO формат compress -H (алгоритм lzh) не использует CRC,
но также допускает некоторые проверки на целостность.
 

Файлы, сжатые  zip -ом, могут быть распакованы gzip-ом только если они содержат один файл, сжатый методом 'deflation'. Эта функция предназначена только для целей конвертирования tar.zip-файлов в формат tar.gz. Для распаковки zip-архивов,
содержащих несколько файлов, вместо  gunzip  используйте  unzip .
...
Сжатие осуществляется всегда, даже если запакованный файл получается немного больше оригинала. В наихудшем случае размер файла увеличивается на несколько байт заголовка gzip, плюс по 5 байт на каждый блок размером 32 Кб, что составляет 0.015% на больших файлах. Обратите внимание, что фактическое число занятых файлом блоков на диске практически никогда не увеличивается.  gzip  при сжатии/распаковке сохраняет права доступа и временные метки файлов.
 ...
 -a --ascii 
Режим текста Ascii: символы конца строки конвертируются в стандарт
данной системы. Эта опция поддерживается только на некоторых
не-Unix системах. В MSDOS последовательность CR LF конвертируется
в LF при сжатии, и LF конвертируется в CR LF при распаковке.
 -c --stdout --to-stdout 
Выводить полученный архив в стандартный поток вывода, оставляя
оригинальные файлы нетронутыми. Если входных файлов несколько, то вывод состоит из последовательности независимо сжатых файлов. Для получения большего сжатия, объедините вместе все входные файлы перед архивированием.
 -d --decompress --uncompress 
Распаковка.
 -f --force 
Проводить сжатие или распаковку, даже если на файл есть ссылки,
такой архив уже существует или сжатые данные читаются или пишутся на терминал. Если входные данные не в формате, поддерживаемом  gzip  и задан ключ --stdout, то они без изменения копируются в стандартный поток вывода: позвольте  zcat  действовать как  cat . Если не указан ключ  -f  и работа не ведётся в фоновом режиме, то  gzip  просит подтвердить, нужно ли переписывать уже существующий файл.
 -h --help 
...
Размер несжатого файла выводится как -1 для файлов не в формате gzip,
таких как .Z-файлы. Чтобы получить размер несжатого файла в таких случаях,
...
Методы сжатия, поддерживаемые на данный момент: deflate, compress, lzh
(compress -H в SCO) и pack.  CRC отображается как ffffffff для файлов
не в формате gzip.
 
С ключом --name имя оригинального файла, дата и время берутся из самого архива, если присутствуют.
 
С ключом --verbose также выводятся общий размер и коэффициент
сжатия всех файлов, при условии что все размеры известны.
С ключом --quiet не показывается заголовок и итоговая часть.
 -L --license 
Показать лицензию  gzip  и выйти.
 -n --no-name 
При сжатии не сохранять по умолчанию имя исходного файла и временные метки (оригинальное имя всегда сохраняется, если имя архива придётся усечь).
При распаковке не восстанавливать имя исходного файла, даже если оно сохранено (только убрать суффикс  gzip  из имени архива), и не восстанавливать оригинальные временные метки (взять их со сжатого файла). Эта опция при распаковке применяется по умолчанию.
 -N --name 
...
Не выводить предупреждения.
 -r --recursive 
Рекурсивно разбирать дерево каталогов. Если в списке из командой строки указан какой-нибудь каталог,  gzip  зайдёт в него и сожмёт все файлы, в нём находящиеся
(или распакует их в случае с  gunzip ).
 -S .суф   --suffix .суф 
...
Формат файла  gzip  описан P. Deutsch в "GZIP file format specification version 4.3",  < ftp://ftp.isi.edu/in-notes/rfc1952.txt > , Internet RFC 1952 (May 1996).
Формат разархивации  zip  описан P. Deutsch в "DEFLATE Compressed Data Format Specification version 1.3",  < ftp://ftp.isi.edu/in-notes/rfc1951.txt > , Internet RFC 1951 (May 1996).
   
 ДИАГНОСТИКА 
...
В командной строке были указаны неправильные ключи.
 file : 
not in gzip format
Файл, указанный  gunzip , не сжат.
 file : 
Corrupt input. Use zcat to recover some data.
Сжатый файл повреждён. Воспользуйтесь zcat для восстановления части данных. Данные до точки повреждения могут быть восстановлены командой
...
compressed with  xx  bits, can only handle  yy  bits
 Файл  был сжат (используя LZW) программой, которая может использовать больше  бит , чем программа распаковки на этом компьютере. Перепакуйте файл gzip, который лучше сжимает и использует меньше памяти.
 file : 
already has .gz suffix -- no change
Файл считается уже сжатым. Переименуйте его и попробуйте снова.
 file  already exists; do you wish to overwrite (y or n)? 
Ответьте "y", если хотите переписать уже существующий архив, в противном случае "n".
 gunzip: corrupt input 
Было зафиксировано нарушение SIGSEGV, что обычно означает, что входной файл повреждён.
 xx.x% 
Сжатием достигнуто уменьшение размера на данную величину. (Относится к ключам  -v  и  -l .)
 -- not a regular file or directory: ignored 
Если указанный файл не является обычным файлом или каталогом (например символическая ссылка, сокет, очередь, файл устройства и т.д.), он не обрабатывается.
 -- has  xx  other links: unchanged 
На указанный файл есть ссылки - он пропускается. Читайте  ln (1) для получения дополнительной информации. Для принудительного сжатия таких файлов используйте ключ  -f .
...
В некоторых редких случаях ключ --best даёт худшее сжатие, чем метод по умолчанию (-6). Некоторые файлы, содержащие чрезвычайно избыточную информацию,  compress 
сжимает лучше, чем  gzip .
   
...
Сopyright ╘ 1992, 1993 Jean-loup Gailly
