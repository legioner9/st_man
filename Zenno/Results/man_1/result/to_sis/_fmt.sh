#!/bin/bash

#. "$HOME/.bashrc"

filename="$HOME/"

echo -e "${HLIGHT}---start file: $filename---${NORMAL}" # start file

#---------------------------------------
:<<end_of_comments


 
 
 
 
 
Профиль:  Аноним  ( вход  |  регистрация ) 

 
 
 
 
 
 

 
 
 
 
  НОВОСТИ  ( + )
 
   КОНТЕНТ 
 
   WIKI 
 
    MAN'ы 
 
    ФОРУМ 
 
 Поиск  ( теги )
 
 
 

 

           
 
 

 
 
 
 
 
 
 
 [  новости  / +++  |  форум  |  wiki  |  теги 
|  
] 
 
 
 
 


 
 
 
 
 
 
 
 
 
 

 Интерактивная система просмотра системных руководств (man-ов) 
  
 
 
 

 
   Тема Набор Категория   
 

  
 Solaris man
 FreeBSD man
 Разные man
 Русские man
 Linux man
 POSIX man
 
 
	   All
	   1
	   2
	   3
	   4
	   5
	   6
	   7
	   8
	   9
 

 
 
  [ Cписок руководств  |  Печать ] 
 
 
 
 fmt (1)   fmt  (1)   ( Solaris man: Команды и прикладные программы пользовательского уровня )   fmt  (1)   ( FreeBSD man: Команды и прикладные программы пользовательского уровня ) >>  fmt  (1)   ( Русские man: Команды и прикладные программы пользовательского уровня )   fmt  (1)   ( Linux man: Команды и прикладные программы пользовательского уровня ) 
   
 ИМЯ 

fmt - утилита для простого форматирование текста
   
 ОБЗОР 

 fmt 

[ -ЦИФРЫ ] [ ОПЦИЯ ]... [ ФАЙЛ ]...
   
 ОПИСАНИЕ 


 

Переформатирует каждый абзац в ФАЙЛАХ, выводя результат на
стандартный вывод.
Если ФАЙЛ не указан или ФАЙЛ задан как `-', читает данные со
стандартного ввода.
 

Обязательные аргументы в длинных опциях обязательны также и для коротких опций.
 
 -c ,  --crown-margin 
сохраняет отступ первых двух строк абзаца.
 -p ,  --prefix = СТРОКА 
форматировать только строки, начинающиеся со СТРОКИ,
сохраняя при этом префикс строк.
 -s ,  --split-only 
просто разделяет длинные строки, не добавляя к ним выравнивающих
пробелов.
 -t ,  --tagged-paragraph 
отступ первой строки отличен от отступа второй.
 -u ,  --uniform-spacing 
один пробел после слова, два после предложения.
 -w ,  --width = ШИРИНА 
максимальная ширина строки (по умолчанию - 75 символов)
 --help 
Вывести справку и выйти
 --version 
Вывести информацию о версии и выйти
 
   
 АВТОР 

Ross Paterson.
   
 СООБЩЕНИЕ ОБ ОШИБКАХ 

Об ошибках сообщайте по адресу < bug-coreutils@gnu.org >.
   
 АВТОРСКИЕ ПРАВА 

Copyright (c) 2006 Free Software Foundation, Inc.
 

Это свободное программное обеспечение. 
Вы можете распространять его копии при соблюдении условий
GNU General Public License < http://www.gnu.org/licenses/gpl.html >.
Не предоставляется НИКАКИХ ГАРАНТИЙ в установленных законом пределах.
   
 СМ. ТАКЖЕ 

Полная документация для
 fmt 

поддерживается в виде руководства в формате Texinfo. Если программы
 info 

и
 fmt 

установлены и настроены в системе, то по команде
 
 
 info fmt 

 
 

вы получите доступ к более полному руководству.
   
 ПЕРЕВОД 

Перевод с английского Alexander Kolotov < alexandr.kolotov@gmail.com >
 

 
   Index 
 
 ИМЯ 
 ОБЗОР 
 ОПИСАНИЕ 
 АВТОР 
 СООБЩЕНИЕ ОБ ОШИБКАХ 
 АВТОРСКИЕ ПРАВА 
 СМ. ТАКЖЕ 
 ПЕРЕВОД 
 
 
 
 
 
 Поиск по тексту MAN-ов:  
 
 
 
 
 
 
 
 
 
 
 


 
 


 
 
 
 
 
Спонсоры:
 
 
 
 
 
 
 
 
Хостинг:
 

 

 


 
 
 
 Закладки на сайте 
 Проследить за страницей 
 
 
Created 1996-2021 by  Maxim Chirkov 
 Добавить ,  Поддержать ,  Вебмастеру 
 
 
 
 


 
 
 






end_of_comments
#---------------------------------------

unset filename