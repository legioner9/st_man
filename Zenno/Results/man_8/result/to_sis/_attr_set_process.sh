...
  
 ...
 attr_set_process (8) >>  attr_set_process  (8)   ( Русские man: Команды системного администрирования ) 
   
 НАЗВАНИЕ 

 

 attr_set_process  - Установка прав процесса.
   
 СИНТАКСИС 

 

 attr_set_process  id-процесса атрибут значение
   
 ОПИСАНИЕ 

 

Программа  attr_set_process  предназначена для установки значения требуемого атрибута для выбранного процесса.
   
 КЛЮЧИ 

 
 -p 
Вывести имена всех результирующих запросов
 -a 
Добавить значение, но не устанавливать
 -m 
Удалить значение, но не устанавливать
...
Атрибут является выражением, значение атрибута - целым числом.
...
Установить идентификатор форсирования роли процесса с PID 9463 в значение 66 (inherit_parent):
 

attr_set_process 9463 rc_force_role 66
...
Александр Блохин < sass@uustoll.ee >
 ...
 RSBAC  - Rule Set Based Access Control,  attr_set_user (8),  attr_set_fd (8),  attr_set_ipc (8),  attr_set_file_dir (8),  attr_set_up (8).
