...
  
 ...
 rpm (8) >>  rpm  (8)   ( Русские man: Команды системного администрирования )   rpm  (8)   ( Linux man: Команды системного администрирования ) Ключ  rpm  обнаружен в базе ключевых слов. 
   
 НАЗВАНИЕ 
...
Необходимо указать один из следующих основных режимов работы:
 ...
 Повторная подпись ,
 Добавление подписи ,
 Задание Владельцев/Групп ,
 Отображение тегов запроса  и
...
Выводить много отладочной информации.
 --rcfile  FILELIST 
Каждый из файлов разделенного двоеточиями списка файлов  FILELIST  последовательно
прочитывается  rpm  на предмет конфигурационной информации.  
В этом списке обязан существовать только первый файл; 
все знаки тильда будут заменены значением  $HOME .
По умолчанию список файлов  FILELIST  выглядит как 
 /usr/lib/rpm/rpmrc : /usr/lib/rpm/redhat/rpmrc : /etc/rpmrc : ~/.rpmrc .
 --pipe  CMD 
Перенаправляет вывод  rpm  на вход команды  CMD .
 --dbpath  DIRECTORY 
Использовать базу данных RPM в каталоге  DIRECTORY  вместо пути по умолчанию
 /var/lib/rpm 
 --root  DIRECTORY 
Использовать для всех операций файловую систему с корнем в  DIRECTORY .  
Обратите внимание, что это значит, что база данных внутри  DIRECTORY  
будет использоваться для проверки зависимостей, и все сценарии  %post  и  %prep  
будут исполняться после  chroot (2) в  DIRECTORY .
...
версия уже установлена. Параметр  PACKAGE_FILE  может быть
указан как адрес 
 ftp  или 
 http  URL,
в таком случае пакет будет скачан перед установкой. См.
 ОПЦИИ FTP/HTTP 
для получения информации о работе  rpm  с поддержкой
 ftp  или
 http .
...
независимо от их существования.
 --badreloc 
Используется вместе с  --relocate , разрешает перемещение
всех путей файлов, не только тех, для которых старые пути  OLDPATH 
...
Используется с  -v|--verbose  для удобства отображения.
 --ignoresize 
...
Она предназначена для упрощения вызова  rpm  из других утилит.
 --prefix  NEWPATH 
...
перемещению (relocation hint) на  NEWPATH .
 --relocate  OLDPATH = NEWPATH 
Для перемещаемых бинарных пакетов, преобразовать все пути файлов,
которые начинаются с  OLDPATH  в заметках по
перемещению (relocation hint) на  NEWPATH .
Данная опция может быть использована несколько раз,
если требуется переместить несколько путей  OLDPATH 
в пакете.
 --repackage 
...
и будет создан в каталоге, названном по значению макроса
 %_repackage_dir  (значение по умолчанию - 
 /var/spool/repackage ).
 --replacefiles 
...
Удалить все версии пакета совпадающие с 
 PACKAGE_NAME . Обычно при наличии нескольких
пакетов совпадающих с  PACKAGE_NAME 
возникает ошибка.
 --nodeps 
...
Наличие параметра  --noscripts  при удалении пакетов
эквивалентно 
...
вслед за которым указывается строка формата  QUERYFMT .  
Форматирование запроса - это измененная версия стандартного 
механизма  printf (3) . Форматирование формируется из 
статических строк (которые могут включать стандартные для 
языка C escape-последовательности для перевода строки, табуляции 
и других спец. символов) и меток форматирования  printf (3) . 
Т.к.  rpm  заранее знает тип выводимой информации, указатель 
формата может быть опущен и заменен на имя выводимого тега заголовка, 
заключенного в фигурные скобки  {} . Имена тегов не чувствительны 
к регистру и префикс  RPMTAG_  в имени тега можно опускать.
 

Альтернативные форматы вывода могут быть заданы при помощи 
задания типа вывода  : typetag  после имени тега.
...
использовать строку формата  %{NAME} .
Для вывода имени пакетов и информации о дистрибутиве в две колонки 
вы можете использовать  %-30{NAME}%{DISTRIBUTION} .
Команда  rpm  отобразит спиков всех тегов, с которыми она может работать 
при ее вызове с параметром  --querytags .
...
и для указания информации.
 ...
 PACKAGE_NAME 
Выполняет запрос к установленному пакету с именем  PACKAGE_NAME .
 -a, --all 
...
т.е.  MD5  дайджест содержимого файла.
 -g, --group  GROUP 
Выполняет запрос к пакету с группой  GROUP .
 --hdrid  SHA1 
Выполняет запрос к пакету, содержащему указанный идентификатор заголовка, 
т.е.  SHA1  дайджест неизменной части заголовка.
 -p, --package  PACKAGE_FILE 
Выполняет запрос к (неустановленному) пакету в файле  PACKAGE_FILE .
Параметр  PACKAGE_FILE  может быть указан в виде адреса  ftp  или 
 http  URL, в результате чего заголовок пакета будет скачан и опрошен.
Обратитесь к  ОПЦИЯМ FTP/HTTP  за информацией о поддержке в
 rpm  работы с 
 ftp  и
 http . 
Если аргумент(ы)  PACKAGE_FILE  не является бинарным пакетом, то он 
...
аргумент  PACKAGE_FILE  в запросе. 
 --pkgid  MD5 
Выполняет запрос к пакету, который содержит указанный идентификатор пакета, 
т.е.  MD5  дайджест объединенного содержимого заголовка и тела пакета.
 --querybynumber  HDRNUM 
Выполняет запрос напрямую  HDRNUM 'ой записи в базе данных; используется 
только для отладки.
 --specfile  SPECFILE 
Обработать и выполнить запрос к  SPECFILE  файлу, как если бы это был 
...
транзакции  TID . В данный момент используется временная метка unix 
...
Будет использована   --queryformat  если указана.
 --last 
...
установки или удаления пакетов.
 -s, --state 
Вывести состояние ( state ) файлов в пакете (подразумевается  -l ). 
Состояние каждого файла в пакете является одним из 
 нормальное (normal) ,
 не установлен (not installed)  или 
 заменен (replaced) .
 --triggers, --triggerscripts 
...
документации, исключенные при помощи опции " --excludedocs ", 
...
внимания выделены жирным), показывающие сбой проверки соответствующего  --verify  теста:
 
 S  размер ( S ize) файла отличается
 M  режим ( M ode) доступа отличается (включая права доступа и тип файла)
 5  отличается контрольная MD 5  сумма
 D  отличается старший/младший номер файла устройства ( D evice)
 L  отличается путь ссылки при read L ink (2) 
 U  отличается владелец ( U ser)
 G  отличается групповое владение ( G roup)
 T  отличается время изменения (m T ime)
...
Опция  --checksig  проверяет все дайджесты и подписи, содержащиеся в 
 PACKAGE_FILE  для проверки целостности и происхождения пакета. 
Обратите внимание, что подписи теперь проверяются при каждом
чтении пакета и опция  --checksig  полезна для проверки
...
данных  rpm  при использовании команды  --import .
...
Обе опции  --addsign  и  --resign 
создают и вставляют новую подпись для каждого указанного
файла  PACKAGE_FILE , заменяя существующие подписи. 
...
Для того, чтобы подписывать пакеты при помощи GPG,  rpm 
должен быть настроен на запуск GPG и иметь возможность 
находить хранилище ключей с требуемыми ключами. По умолчанию
 rpm  использует те же соглашения, что и GPG для обнаружения 
хранилища ключей, а именно переменную среды  $GNUPGHOME .
...
из хранилища ключей расположенного в  /etc/rpm/.gpg ,
используя программу  /usr/bin/gpg , вам потребуется включить строки
...
показывает значения, которые  rpm  будет использовать
для всех опций, установленные в конфигурационных файлах
 ...
 rpm  может выступать в качестве клиента FTP и/или HTTP,
...
Если параметр  :PASSWORD  будет опущен, пароль будет запрошен
(один раз для пары пользователь/узел). Если опущены оба параметра
(user и password), будет выполнено анонимное подключение  ftp .
Во всех случаях используется пассивный (PASV) режим передачи 
файлов  ftp .
 

 rpm  допускает следующие опции для адресов
ftp URL:
 
 --ftpproxy  HOST 
Узел  HOST  будет использован как прокси-сервер
...
настройке макроса  %_ftpproxy . 
 --ftpport  PORT 
Номер TCP порта  PORT  будет использован для ftp подключения
к ftp прокси-серверу вместо порта по умолчанию. Эта опция
может быть также указана при настройке макроса  %_ftpport .
 ...
 --httpproxy  HOST 
Узел  HOST  будет использован как прокси-сервер для
всех операций передачи данных  http . Эта опция
может быть также указана при настройке макроса
 %_httpproxy .
 --httpport  PORT 
Номер TCP порта  PORT  будет использован для  http  подключения
к http прокси-серверу вместо порта по умолчанию. Эта опция
может быть также указана при настройке макроса  %_httpport .
...
удалена из rpm. Установите пакет  rpmbuild  и просмотрите
документацию в  rpmbuild (8) по всем режимам сборки  rpm ,
ранее приведенную здесь в  rpm (8). 
 

Добавьте следующие строки в  /etc/popt , 
если вы желаете вызывать  rpmbuild  из 
командной строки  rpm :
