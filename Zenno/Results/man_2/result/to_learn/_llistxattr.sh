...
  
 ...
 llistxattr (2) >>  llistxattr  (2)   ( Русские man: Системные вызовы )   llistxattr  (2)   ( Linux man: Системные вызовы ) 
   
 НАЗВАНИЕ 
...
названий расширенных атрибутов, ассоциированных с заданным путем
 path 

в файловой системе.
Список является набором названий (заканчивающихся NULL) один за одним.
Названия расширенных атрибутов, к которым вызывающий процесс не может
иметь доступ, могут быть исключены из списка.
Возвращается длина списка
 list 
...
за исключением случая символьной ссылки, где список названий расширенных атрибутов
ассоциируется с самой полученной ссылкой, а не с файлом, на который они ссылаются.
...
названий возвращается в виде неупорядоченного массива
символьных строк, заканчивающихся NULL (названия атрибутов
разделяются символами NULL), например:
...
Файловые системы ext2, ext3 и XFS, реализующие ACL стандарта POSIX,
используя расширенные атрибуты, должны возвращать список
 list 
...
обозначая размер списка названий расширенных атрибутов.
