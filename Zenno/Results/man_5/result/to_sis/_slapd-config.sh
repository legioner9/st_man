...
  
 ...
 slapd-config (5) >>  slapd-config  (5)   ( Русские man: Форматы файлов ) 
...
(без префикса  rwm- ). Не следует смешивать параметры
 olcAuthIDRewrite 
...
которые требуются для поставщика репликации syncrepl. Значение по умолчанию - TRUE.
...
наложения  glue  в нужной позиции. Явное указание требуется, например, при использовании
...
в настройках сервера-потребителя репликации. Это неотрицательное целое число не более трёх десятичных цифр.
...
а для  searchbase  нет значения по умолчанию. Значение по умолчанию для списка атрибутов
 ...
 attrsonly  по умолчанию не установлен. Параметры  sizelimit  и  timelimit 
в качестве значений принимают только ключевое слово "unlimited" и положительное целое число,
значение по умолчанию для обоих - "unlimited". Имейте ввиду, что на стороне поставщика,
...
пока повторное подключение не будет успешно установлено.
...
На поставщике репликации, помимо того, чтобы разрешить идентификационной сущности syncrepl проходить
аутентификацию, также следует предоставить этой идентификационной сущности соответствующие привилегии доступа к данным,
которые будут реплицироваться (директива  olcAccess ), а также установить подходящие ограничения по времени
и по размеру (директива  olcLimits ).
...
/usr/local/etc/openldap/slapd.d. Тогда можно проинициализировать конфигурацию такой командой:
