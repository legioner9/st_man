...
sha1-old and sha1-new should be valid objects in the repository. 
 ...
 When accepting a signed push (see  git-push[1] ), the signed
...
family of commands (see  git-log[1] ). 
...
challenging. 
 ...
 Any objects created by the  pre-receive  hook will be created in
the quarantine directory (and migrated only if it succeeds). 
 
 
 The  pre-receive  hook MUST NOT update any refs to point to
