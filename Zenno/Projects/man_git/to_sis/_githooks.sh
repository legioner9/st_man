...
need to be resolved and the result committed separately (see
 git-merge[1] ). At that point, this hook will not be executed,
...
these two commands. 
 ...
 It is also run after  git-clone[1] , unless the  --no-checkout  ( -n ) option is
...
for an example of how to do this. 
 ...
 This hook is called by  git-push[1]  and can be used to prevent
...
( git-commit[1]  when called with  --amend  and
