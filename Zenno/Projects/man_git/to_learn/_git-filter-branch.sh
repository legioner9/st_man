...
the replacement commit created by  git-commit-tree[1]  after the
filters have run. 
...
the  git filter-branch  options. Implies  Remap to ancestor . 
 ...
 By using  git-rev-list[1]  arguments, e.g., path limiters, you can limit the
...
will not have the removed objects.  See  git-clone[1] .  (Note
